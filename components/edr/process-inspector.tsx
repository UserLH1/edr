...
import { X, ChevronRight, Skull, PauseCircle, Loader2, AlertTriangle } from "lucide-react"
...
function getMitreTactics(proc: ProcessInfo): string[] {
  ...
  if (cmd.includes("-enc") || cmd.includes("-encodedcommand"))
    tactics.push("[T1059.001] Encoded PowerShell")
  if (cmd.includes("-windowstyle hidden") || cmd.includes("-w hidden"))
    tactics.push("[T1564.003] Hidden Window")
  if (cmd.includes("-noprofile") || cmd.includes("-nop "))
    tactics.push("[T1059.001] PowerShell NoProfile")
  ...
  if (exe.includes("\\temp\\") || exe.includes("\\appdata\\"))
    tactics.push("[T1036.005] Masquerading / Temp Path")
  if (name === "nc.exe" || name === "ncat.exe" || name === "netcat.exe")
    tactics.push("[T1059.004] Netcat / Reverse Shell")
  if (name.includes("mimikatz") || name.includes("mimi"))
    tactics.push("[T1003] OS Credential Dumping")
  if (cmd.includes("downloadstring") || cmd.includes("webclient") || cmd.includes("wget") || cmd.includes("curl"))
    tactics.push("[T1105] Ingress Tool Transfer")
  if (cmd.includes("invoke-expression") || cmd.includes("iex "))
    tactics.push("[T1059.001] PowerShell IEX")
...
interface ProcessInspectorProps {
  /** The selected process — null when opened before data loads */
  process: ProcessInfo | null
  /** Full process list — used to render the execution lineage tree */
...
export function ProcessInspector({ process: proc, processes, onClose }: ProcessInspectorProps) {
  ...
  // Fetch network connections whenever the inspected process changes
  useEffect(() => {
    if (!proc || !isTauriRuntime()) {
      setConnections([])
      return
    }
    ...
    ipc
      ...
      .catch(err => console.error("[NexusEDR] getNetworkConnections FAILED:", err))
  ...
  async function handleKill() {
    ...
    setActionError(null)
    try {
      await ipc.killProcess(proc.pidNum)
      onClose()
    } catch (err: unknown) {
  ...
  async function handleSuspend() {
    ...
    setActionError(null)
    try {
      await ipc.suspendThread(proc.pidNum)
    } catch (err: unknown) {
      setActionError(err instanceof Error ? err.message : String(err))
    } finally {
  ...
  // Build execution lineage: walk up the parent chain from proc
        ...
        <div className="flex flex-col gap-0">
          ...
          <div className="px-4 pt-4 pb-3 border-b border-zinc-800/60">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={onClose}
                className="text-zinc-500 hover:text-zinc-200 transition-colors rounded p-0.5 hover:bg-zinc-800"
              ...
              <>
                ...
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  <span
                    ...
                    aria-label={`Threat score ${proc.threatScore}, ${proc.threatLevel.toUpperCase()}`}
                  >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    {killPending ? (
                      ...
                      <Skull size={12} className="mr-1.5" />
                    )}
                    KILL PROCESS
                  </Button>
                  <Button
                  ...
                  >
                    {suspendPending ? (
                      ...
                      <PauseCircle size={12} className="mr-1.5" />
                    )}
                    Suspend Threads
                  </Button>
                </div>
          ...
          {/* ── Tabs (only rendered when a process is selected) ─── */}
          {proc && (
            <Tabs defaultValue="overview" className="flex-1 gap-0">
              ...
              <TabsContent value="overview" className="mt-0 p-4 flex flex-col gap-5">
                ...
                <div>
                  ...
                  <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-xs font-mono">
                    ...
                    <MetaRow
                      label="Parent PID"
                      value={proc.parentPid != null ? String(proc.parentPid) : "—"}
                    />
                    <MetaRow label="CPU" value={`${proc.cpuPercent.toFixed(1)}%`} />
                    <MetaRow
                      label="Memory"
                      value={`${proc.memMb.toFixed(1)} MB`}
              ...
              <TabsContent value="network" className="mt-0 p-4">
                <SectionHeader>Active Connections</SectionHeader>
                ...
                {connsLoading ? (
                  ...
                  <p className="text-zinc-600 text-xs font-mono py-4 text-center">
                    No connections found for PID {proc.pidNum}.
                  </p>
                ) : (
                  <div className="flex flex-col gap-1.5">
                    {connections.map((c, i) => {
                      const isEstab = c.state === "ESTABLISHED"
                      return (
                        <div
                          key={i}
                          className="flex items-center gap-2 p-2 rounded bg-zinc-900/60 border border-zinc-800/50 font-mono text-[11px]"
                        >
              ...
              <TabsContent value="execution" className="mt-0 p-4">
                ...
                <div className="flex flex-col gap-1 font-mono text-[11px]">
                  {lineage.map((p, idx) => {
                    ...
                    const prefix = idx === 0 ? "" : idx === lineage.length - 1 ? "└── " : "├── "
                    const color = p.threatLevel === "critical"
                      ? "text-red-400"
                      : p.threatLevel === "warning"
                      ? "text-yellow-400"
                      : "text-zinc-300"
                    return (
                      <div key={p.pidNum} className="flex items-baseline gap-1">
                        ...
                        {isCurrent && (
                          <span className="text-cyan-500 text-[9px] ml-1">◄ CURRENT</span>
                        )}
                      </div>
                    )
                  })}
